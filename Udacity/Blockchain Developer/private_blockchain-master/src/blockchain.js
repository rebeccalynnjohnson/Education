...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            let timeFromMsg = parseInt(message.split(':')[1]);
            let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
            const FIVE_MINS = 5 * 60 * 1000;
            if ((timeFromMsg + FIVE_MINS) >= currentTime) {
                let verified = bitcoinMessage.verify(message, address, signature);

                if (verified) {
                    let newBlock = new BlockClass.Block({ owner: address, star: star });
                    const block = this._addBlock(JSON.stringify(newBlock));
                    resolve(block);
                 }
                 
            } else {
                reject(new Error('Could not create the block'));
            }
        });
    }
